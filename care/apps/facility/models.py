...
class Ambulance(commons_models.SoftDeleteTimeStampedModel):
    ...
    insurance_valid_till_year = models.IntegerField(
        choices=commons_facility_constants.INSURANCE_YEAR_CHOICES
    )
    ambulance_type = models.IntegerField(
        choices=commons_facility_constants.AMBULANCE_TYPES, blank=False, default=1
    )
...
class HospitalDoctors(commons_models.SoftDeleteTimeStampedModel):
    ...
    class Meta:
        indexes = [
            PartialIndex(
                fields=["facility", "area"], unique=True, where=PQ(active=True)
            )
        ]
...
class FacilityCapacity(commons_models.SoftDeleteTimeStampedModel):
    ...
    class Meta:
        indexes = [
            PartialIndex(
                fields=["facility", "room_type"], unique=True, where=PQ(active=True)
            )
        ]
