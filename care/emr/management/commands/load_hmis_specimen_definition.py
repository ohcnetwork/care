...
class Command(BaseCommand):
    ...
    - type_collected_code (required)
    - type_collected_system (required)
    - type_collected_display (required)
    - patient_preparation (optional, comma-separated values)
    - collection_code (optional)
    - collection_system (optional)
    - collection_display (optional)
    - is_derived (optional, yes/no, default: no)
    - preference (optional, default: preferred)
    - container_description (optional)
    - container_capacity (optional)
    - container_capacity_unit_code (optional)
    - container_capacity_system (optional)
    - container_capacity_unit_display (optional)
    - container_minimumvolume (optional, numeric value)
    ...
    - single_use (optional, yes/no, default: yes)
    """
    ...
    def parse_yes_no_to_bool(self, value, default: bool = False) -> bool:
        """Parse yes/no/true/false string to boolean."""
        ...
        if str_value in ("yes", "true", "1"):
            return True
        if str_value in ("no", "false", "0"):
            return False
        return default

    def parse_patient_preparation(self, value) -> list:
        """Parse patient_preparation as a list (comma-separated in CSV)."""
        if not value:
            return []
    ...
    def process_row(self, row: dict, facility: Facility) -> dict:
        ...
        try:
            ...
            collection = parse_code(
                row.get("collection_code"),
                row.get("collection_system"),
                row.get("collection_display"),
            )

            patient_preparation = self.parse_patient_preparation(
                row.get("patient_preparation")
            )
            ...
            # Parse capacity with unit as coded object
            capacity_unit = parse_code(
                row.get("container_capacity_unit_code"),
                row.get("container_capacity_system"),
                row.get("container_capacity_unit_display"),
            )
            ...
            container_description = row.get("container_description")
            container_preparation = row.get("container_preparation")
            ...
            if container_preparation:
                container["preparation"] = container_preparation

            # Handle capacity with proper unit structure
            container_capacity = row.get("container_capacity")
            if container_capacity:
                try:
                    capacity_value = float(container_capacity)
                    if capacity_value > 0:
                        capacity_obj = {"value": str(capacity_value)}
                        if capacity_unit:
                            capacity_obj["unit"] = capacity_unit
                        container["capacity"] = capacity_obj
                except (ValueError, TypeError):
                    pass

            # Handle minimum_volume - support both string and quantity formats
            minimum_volume_string = row.get("container_minimumvolume_string")
            minimum_volume = row.get("container_minimumvolume")

            if minimum_volume_string:
                # Use string format (e.g., "3-5ml")
                container["minimum_volume"] = {"string": minimum_volume_string}
            elif minimum_volume and minimum_volume_unit:
                # Use quantity format
                try:
            ...
            if row.get("retention_time_value") and retention_time_unit:
                try:
                    retention_time = {
                        "value": str(float(row["retention_time_value"])),
            ...
            type_tested = {
                "is_derived": self.parse_yes_no_to_bool(row.get("is_derived"), False),
                "preference": row.get("preference", "preferred"),
                "retention_time": retention_time,
                "single_use": self.parse_yes_no_to_bool(row.get("single_use"), True),
            }
    ...
    def create_specimen_definition(
        ...
        try:
            ...
            if existing:
                logger.warning(
                    "Specimen definition already exists with slug: %s (title: %s)",
                    data["slug_value"],
                    data["title"],
                )
                return existing
