...
class FacilityInventoryUnitConverter(models.Model):
    ...
    from_unit = models.ForeignKey(
        FacilityInventoryUnit, related_name="from_unit", on_delete=models.CASCADE, null=False, blank=False
    )
    to_unit = models.ForeignKey(
        FacilityInventoryUnit, related_name="to_unit", on_delete=models.CASCADE, null=False, blank=False
    )
...
class FacilityInventoryItem(models.Model):
    ...
    default_unit = models.ForeignKey(
        FacilityInventoryUnit, related_name="default_unit", on_delete=models.SET_NULL, null=True, blank=False
    )
...
class FacilityInventorySummary(FacilityBaseModel, FacilityRelatedPermissionMixin):
    ...
    facility = models.ForeignKey("Facility", on_delete=models.CASCADE, null=False, blank=False)
    item = models.ForeignKey(FacilityInventoryItem, on_delete=models.SET_NULL, null=True, blank=False)
    quantity = models.FloatField(default=0)  # Automatically Set // NOT EDITABLE BY ADMIN
