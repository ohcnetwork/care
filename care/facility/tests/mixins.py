...
class TestClassMixin(TestCase):
    def setUp(self):
        ...
        district = District.objects.create(state=state, name="Ernakulam")
        local_body = LocalBody.objects.create(
            district=district, name="Test Local Body", body_type=20, localbody_code="T1"
        )
        ward = Ward.objects.create(local_body=local_body, name="Test Ward", number=1)
        self.users = [
    ...
    def new_request(self, params, method, viewset, authenticate=False, pk=None):
        ...
        if authenticate:
            force_authenticate(request, user=authenticate)
