...
class PatientNotesTestCase(TestBase, TestClassMixin, APITestCase):
    ...
    def setUp(self):
        ...
        self.user.home_facility = self.facility
        self.user.save()
        ...
        self.facility2 = self.create_facility(district=district, user=self.user2)
        self.user2.home_facility = self.facility2
        self.user2.save()

        self.patient = self.create_patient(district=district.id, facility=self.facility)
        ...
        self.consultation2 = self.create_consultation(
            patient=self.patient, facility=self.facility2
        )
        self.patient.last_consultation = self.consultation2
        self.patient.save()
    ...
    def test_patient_notes(self):
        ...
        # Test user_type field if user is from other facility as patient
        data2 = results[0]
