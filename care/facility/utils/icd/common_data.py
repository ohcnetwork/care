...
def find_roots(item, roots_lookup):
    id = item["ID"]
    if id in roots_lookup:
        return roots_lookup[id]

    if not item["parentId"]:
        roots_lookup[id] = {item["classKind"]: item["label"]}
        return roots_lookup[id]

    if parent := roots_lookup.get(item["parentId"]):
