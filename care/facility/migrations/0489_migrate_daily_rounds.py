...
human_body_map = {
    ...
    "AnteriorRightShoulder": {
        "system": "http://snomed.info/sct",
        "code": "368106002",
        "display": "Right Shoulder structure",
    },
    "AnteriorRightChest": {
        "system": "http://snomed.info/sct",
        "code": "771319006",
            ...
            if daily_round.infusions:
                ...
                for daily_round_infusion in daily_round.infusions:
                    if infusion := daily_round_infusion.get("infusion"):
                        code = infusions_map.get(infusion.strip())
                        if not code:
                            logger.warning(
                                f"Infusion code not found for {infusion.strip()}"
                            )
                            continue
                        values.append(
                            {
            ...
            if daily_round.iv_fluids:
                ...
                for daily_round_iv_fluid in daily_round.iv_fluids:
                    if iv_fluid := daily_round_iv_fluid.get("iv_fluid"):
                        code = iv_fluids_map.get(iv_fluid.strip())
                        if not code:
                            logger.warning(
                                f"Infusion code not found for {iv_fluid.strip()}"
                            )
                            continue
                        values.append(
                            {
            ...
            if daily_round.feeds:
                ...
                for daily_round_feed in daily_round.feeds:
                    if feed := daily_round_feed.get("feed"):
                        code = feeds_map.get(feed.strip())
                        if not code:
                            logger.warning(
                                f"Feed code not found for {feed.strip()}"
                            )
                            continue
                        values.append(
                            {
            ...
            if daily_round.output:
                ...
                for daily_round_output in daily_round.output:
                    if output_name := daily_round_output.get("name"):
                        code = output_map.get(output_name.strip())
                        if not code:
                            logger.warning(
                                f"Output code not found for {output_name.strip()}"
                            )
                            continue
                        values.append(
                            {
            ...
            if daily_round.nursing:
                ...
                for daily_round_nursing in daily_round.nursing:
                    if nursing_procedure := daily_round_nursing.get("procedure"):
                        code = nursing_map.get(nursing_procedure.strip())
                        if not code:
                            logger.warning(
                                f"Nursing code not found for {nursing_procedure.strip()}"
                            )
                            continue
                        values.append(
                            {
