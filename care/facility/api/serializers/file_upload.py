...
def check_permissions(  # noqa: PLR0911, PLR0912
    file_type,
    associating_id,
    user,
    action="create",
    ...
    try:
        ...
        if file_type == FileUpload.FileType.SAMPLE_MANAGEMENT.value:
            ...
            if not has_facility_permission(user, patient.facility):
                raise Exception("No Permission")
            return sample.id
        if file_type == FileUpload.FileType.CLAIM.value:
            return associating_id
        if file_type == FileUpload.FileType.COMMUNICATION.value:
            return associating_id
        raise Exception("Undefined File Type")

    except Exception:
        raise serializers.ValidationError({"permission": "denied"}) from None
