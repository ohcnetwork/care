...
class PatientConsultationListSerializer(serializers.ModelSerializer):
    id = serializers.CharField(source="external_id", read_only=True)
    facility_name = serializers.CharField(source="facility.name", read_only=True)
    facility = serializers.CharField(source="facility.external_id", read_only=True)
    patient = serializers.CharField(source="patient.external_id", read_only=True)
    last_edited_by = UserBaseMinimumSerializer(read_only=True)
    suggestion_text = ChoiceField(
        choices=PatientConsultation.SUGGESTION_CHOICES,
        read_only=True,
...
class PatientConsulationDetailSerializer(PatientConsultationListSerializer):
    id = serializers.CharField(source="external_id", read_only=True)
    facility_name = serializers.CharField(source="facility.name", read_only=True)
    suggestion_text = ChoiceField(
        choices=PatientConsultation.SUGGESTION_CHOICES,
