...
class ShiftingSerializer(serializers.ModelSerializer):
    ...
    patient_object = PatientListSerializer(
        source="patient", read_only=True, required=False
    )
    ...
    patient = serializers.UUIDField(
        source="patient.external_id", allow_null=False, required=True
    )
    ...
    def update(self, instance, validated_data):
        ...
        LIMITED_RECIEVING_STATUS = [
            REVERSE_SHIFTING_STATUS_CHOICES[x] for x in LIMITED_RECIEVING_STATUS_
        ]
        ...
        LIMITED_SHIFTING_STATUS = [
            REVERSE_SHIFTING_STATUS_CHOICES[x] for x in LIMITED_SHIFTING_STATUS_
        ]
        ...
        RECIEVING_REQUIRED_STATUS = [
            REVERSE_SHIFTING_STATUS_CHOICES[x] for x in RECIEVING_REQUIRED_STATUS_
        ]
            ...
            if validated_data["status"] in RECIEVING_REQUIRED_STATUS:
                if (
                    not (
                        ...
                        or instance.assigned_facility_external
                    )
                ) and (not assigned):
