...
class ScheduleAccess(AuthorizationHandler):
    def can_write_schedule(self, resource_obj, user):
        if (
            resource_obj.resource_type
            == SchedulableResourceTypeOptions.practitioner.value
        ):
            return self.can_write_practitioner_schedule(
        ...
        if (
            ...
            return self.can_write_healthcare_service_schedule(
        ...
        if resource_obj.resource_type == SchedulableResourceTypeOptions.location.value:
            return self.can_write_location_schedule(
                resource_obj.location, user, resource_obj.facility
            )
        raise ValueError("Invalid resource type")

    def can_list_schedule(self, resource_obj, user):
        if (
            resource_obj.resource_type
            == SchedulableResourceTypeOptions.practitioner.value
        ):
            return self.can_read_practitioner_schedule(
                resource_obj.user, user, resource_obj.facility
            )
        if (
            resource_obj.resource_type
            ...
            return self.can_read_healthcare_service_schedule(
                resource_obj.healthcare_service, user, resource_obj.facility
            )
        if resource_obj.resource_type == SchedulableResourceTypeOptions.location.value:
            return self.can_read_location_schedule(
                resource_obj.location, user, resource_obj.facility
            )
        raise ValueError("Invalid resource type")

    def can_write_practitioner_schedule(self, obj, user, facility):
        ...
        return self.check_permission_in_facility_organization(
            [SchedulePermissions.can_write_schedule.name], user, orgs=cache
        )

    def can_write_healthcare_service_schedule(self, obj, user, facility):
        """
        ...
        if obj.managing_organization:
            ...
            return self.check_permission_in_facility_organization(
                [SchedulePermissions.can_write_schedule.name], user, orgs=orgs
            )
        return self.check_permission_in_facility_organization(
    ...
    def can_write_location_schedule(self, obj, user, facility):
        return self.check_permission_in_facility_organization(
            [SchedulePermissions.can_write_schedule.name],
            user,
    ...
    def can_read_practitioner_schedule(self, obj, user, facility):
        facility_orgs = FacilityOrganizationUser.objects.filter(
            user=obj, organization__facility=facility
        ).values("organization__parent_cache", "organization_id")
        cache = []
        ...
        cache = list(set(cache))
        return self.check_permission_in_facility_organization(
            [SchedulePermissions.can_list_schedule.name], user, orgs=cache
        )

    def can_read_healthcare_service_schedule(self, obj, user, facility):
        """
        Anyone in the managing organization of the healthcare service can write the schedule
        """
        if obj.managing_organization:
            ...
            return self.check_permission_in_facility_organization(
                [SchedulePermissions.can_list_schedule.name], user, orgs=orgs
            )
        return self.check_permission_in_facility_organization(
    ...
    def can_read_location_schedule(self, obj, user, facility):
        return self.check_permission_in_facility_organization(
            [SchedulePermissions.can_list_schedule.name],
            user,
            orgs=obj.facility_organization_cache,
...
AuthorizationController.register_internal_controller(ScheduleAccess)
