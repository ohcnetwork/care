    ...
    def handle(self, *args, **options):
        ...
        for item in data:
            ...
            if state_name.lower() in states_to_ignore:
                self.stdout.write(self.style.WARNING(f"Skipping {state_name}"))
                continue
            ...
            self.stdout.write(
                f"{'Created' if is_created else 'Retrieved'} state {state_name}",
            )
            ...
            for d in districts:
                ...
                self.stdout.write(
                    f"{'Created' if is_created else 'Retrieved'} district {d}",
                )
