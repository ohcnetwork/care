...
class TestSuperUser(TestUserBase):
    ...
    def test_superuser_can_modify(self):
        ...
        response = self.client.put(self.get_url(self.user.username), {**data, "age": 31, "password": password},)
...
class TestUser(TestUserBase):
    ...
    def test_user_can_modify_themselves(self):
        ...
        password = "new_password"
        username = self.user.username
        response = self.client.patch(self.get_url(self.user.username), {"age": 31, "password": password,},)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        # test the value from api
        self.assertEqual(response.json()["age"], 31)
        # test value at the backend
